...
function LangList({ choise1, choise2, Permute, isHide, takeLang, isOpen1, isOpen2 }) {
    ...
    return (
        <div className={`w-1/2 flex-row gap-4 pt-0 pb-4 relative ${(isHide === true) ? 'hidden' : 'flex'} `}>
            <Select
                options={GetData()[0]}
                ...
                isSearchable={true}
                menuIsOpen={true}
                classNames={`${(isOpen1 == false) ? 'hidden' : 'block'} `}
            />
            <Select
                options={GetData()[0]}
                ...
                isSearchable={true}
                menuIsOpen={true}
                classNames={`${(isOpen2 == false) ? 'hidden' : 'block'} `}
            />
        </div>
    );
}
