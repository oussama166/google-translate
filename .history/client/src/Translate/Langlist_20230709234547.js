...
function LangList({ choise1, choise2, Permute, isHide, takeLang, VoiceReg }) {
    function GetData() {
        let options = [];
    ...
    const handleChange1 = value => {
        ...
        takeLang(prevState => {
            ...
            return { ...rest, from: value.value || "en" };
        })
        VoiceReg(prevState => {
            const { from: _, ...rest } = prevState; // Destructure "from" property and ignore it
            return { ...rest, from: value }
        })
    };
    ...
    return (
        <div
            className={`w-1/2 flex-row gap-4 pt-0 pb-4 relative ${(isHide === true) ? 'hidden' : 'flex'} transition-transform ease-in-out`}>
            <Select
                options={GetData()[0]}
