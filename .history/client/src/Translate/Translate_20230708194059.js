...
const Translate = () => {
    ...
    const [getTranslate, setGetTranslate] = useState(["My name is oussama", "en", "ar"])
        ...
        <div className='w-full h-screen bg-slate-50 flex flex-col z-[1] '>
            <div
                ...
                <div className='w-1/2 max-h-14 min-h-14 h-14 bg-[#316aba] flex flex-row justify-between items-center rounded-md px-5 font-google flex-shrink-0'>
                    <span className='cursor-pointer max-w-[120px]'>{langChoise.label}</span>
                    <HiSwitchHorizontal
                        size={24}
                        className=' cursor-pointer hover:animate-spin'
                        onClick={changeLang}
                    />
                    <span className='cursor-pointer max-w-[120px]'>{langChoise2.label}</span>
                </div>
